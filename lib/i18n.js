...
(function (root) {
  ...
  const MESSAGES = {
    en: {
      ...
      "tools.model": "Model",
      "tools.modelHint":
        "Defaults to the model selected in the chat window. Pick another fetched model to always use it for this tool.",
      "tools.sameAsChatModel": "Same as chat",
      "tools.modelUnavailable": "Unavailable",
      ...
      "misc.hint":
        "Export saves all settings, including API keys, models, and every tool (name, prompt, and model). Chat history is not included. Import restores them and reloads the sidebar.",
    ...
    zh: {
      ...
      "tools.model": "模型",
      "tools.modelHint":
        "默认使用聊天窗口中选中的模型。也可以选择其它已获取的模型，让这个工具始终使用它。",
      "tools.sameAsChatModel": "与聊天相同",
      "tools.modelUnavailable": "不可用",
      ...
      "misc.import": "导入设置备份",
      "misc.hint":
        "导出会保存所有设置（含 API 密钥、模型，以及每个工具的名称、提示词和模型），但不包含聊天记录。导入会恢复这些设置并重新加载侧边栏。",
